...
def main():
    db = "sensorRecord.db"
    connection = connectionCreation(db)
    genericQuerie(connection, '''DROP TABLE IF EXISTS Probe''')
    genericQuerie(
        connection, '''CREATE TABLE Probe ( ID INTEGER PRIMARY KEY, Name TEXT NOT NULL, Value TEXT NOT NULL, Date TEXT NOT NULL )''')
    with connection:
        fullLine = ""
        ...
        for line in db:
            ...
            if nbcontents >= 15:
                fullLine = ""
                insertInto(connection, [int(
                    contents[0][1:-1]), contents[2][1:-1], contents[3][1:-1], contents[7][1:-1]])
    connection.commit()
    print("Success ! All data are inserted")
